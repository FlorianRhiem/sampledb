...
def handle_authentication(url: str, token: str) -> tuple[users.User, str]:
    ...
    if flask.current_app.config['OIDC_USE_SESSION'] and not token_response.refresh_token:
        raise RuntimeError(
            'OIDC_USE_SESSION is set, but the OP has not set a refresh token'
        )
...
def handle_backchannel_logout(logout_token: str) -> None:
    ...
    login_sessions = [
        ...
        authentication.get_active_login_sessions(authentication_method.user_id)
