...
def test_get_all_object_permissions(flask_server, auth, user, other_user, object_id):
    ...
    assert r_all.json()["basic_groups"] == r_groups.json()
    assert r_all.json()["projects"] == r_projects.json()
    assert r_all.json()["authenticated"] == r_authenticated.json()
    assert r_all.json()["anonymous"] == (r_anonymous.json() if r_anonymous.ok else 'none')
...
def test_set_object_permissions(flask_server, auth, user, other_user, object_id):
    ...
    assert all([
        r.json()["basic_groups"][str(group_ids[0])] == 'write',
        r.json()["basic_groups"].get(str(group_ids[1])) == None,
        r.json()["basic_groups"][str(group_ids[2])] == 'read',
    ])
    assert all([
    ...
    assert r.json()["authenticated"] == 'read'
    assert r.json()["anonymous"] == 'read'
    ...
    r = requests.put(flask_server.base_url + 'api/v1/objects/{}/permissions'.format(object_id), json=request_json, auth=auth)
    assert r.status_code == 200
    assert r.json()["authenticated"] == 'none'
    assert r.json()["anonymous"] == 'none'
